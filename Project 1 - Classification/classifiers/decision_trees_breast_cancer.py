...
def perform_decision_trees_on_breast_cancer(preprocess_method: str=None,
    ...
    if not preprocess_method:
        print('Using raw dataset for breast cancer decision_trees...')
        more_info_name += '_raw'
        x_train = x_train[columns_without_ID]
        x_test = x_test[columns_without_ID]

    elif preprocess_method == 'normalize':
        print('Using normalized dataset for breast cancer decision_trees...')
    ...
    if not feature_selection:
        ...
        use_features = get_best_features_with_decision_tress(
            x_train[columns_without_ID], y_train, dataset_name=breast_cancer_data_folder / 'breast_cancer_decision_trees')
    ...
    if not search_params:
        ...
        try:
            ...
            prediction_dataframe.to_csv(
                breast_cancer_data_folder / 'grid_search_decision_trees_breast_cancer_{}{}.csv'.format(more_info_name,
                                                                                            timestamp), index=False)
            print('Stored file as: grid_search_decision_trees_breast_cancer_{}{}.csv'.format(more_info_name, timestamp))
...
def main(preprocess_method, feature_selection, feature_param, search_params):
    ...
    perform_decision_trees_on_breast_cancer(preprocess_method=preprocess_method,
